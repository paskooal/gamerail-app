...
@section('title', 'Listar Categorias de Produtos')
@section('devarea')
...
@section('content')
 ...
 cursor-pointer rounded-md p-3 transition-all hover:bg-white hover:border-b-4 border-box
  hover:border-purple-700 hover:text-purple-700 justify-center" href="{{route('prodCategories.create')}}">
                    <i class="bx bx-plus text-2xl"></i>Adicionar Categoria de Produto</a>
        
            @if($totalProdCategories > 0)
        <p class="text-purple-800 text-xl">Categorias de Jogos Totais: {{$totalProdCategories}}</p>
        </div>
        <div class="overflow-auto rounded-md shadow-md sm:mx-2">
            ...
            <tbody class="divide-y divide-purple-500">
                ...
                <tr class="divide-x divide-purple-500 bg-purple-300 text-purple-800">
                                        <td class="p-3 font-bold">{{ $prodCategory->id }}</td>
                                        <td class="p-3 font-bold truncate">{{$prodCategory->name}}</td>
                    <td class="p-1 font-bold truncate max-w-32">{{$prodCategory->description}}</td>
                        ...
                        <a class="cursor-pointer rounded-md hover:scale-110 transition-all
                         hover:bg-purple-700 bg-purple-600 ml-4 my-1 px-2 p-1 text-white bx bx-edit-alt
                          text-xl" href="/prodCategories/edit/{{$prodCategory->id}}">
                        </a>
</td>


                        <td class="font-bold justify-center m-2 items-center">
                            <form method="POST" action="/prodCategories/destroy/{{$prodCategory->id}}">
                    @csrf
                    @method('DELETE')
