...
@section("title", "Editanto " . $prodCategory->name)
@section("content")
    <form id="form" class="" data-parsley-validate action="{{ route('prodCategories.update', $prodCategory->id) }}" method="POST">
        @csrf
        @method('PUT')
        <input type="hidden" name="id" value="{{ $prodCategory->id }}">
        <div class="md:flex md:gap-6">
        <div class="flex-1 mb-4">
            <x-input-label for="name" :value="__('Nome')" />
            <x-text-input required maxLength="30" minLength="2" id="name" name="name" value="{{ old('name', $prodCategory->name) }}" type="text" class="w-full" />
        </div>
        </div>
            ...
            <textarea required maxLength='200' minLength='2'
                ...
                name="description"
                type="text"
            >{{ old('description', $prodCategory->description) }}</textarea>
        ...
        <x-link
            ...
            href="{{route('prodCategories.index')}}"
